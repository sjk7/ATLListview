...
public:
	...
	virtual ~CListControl() {
		...
		if (m_phdrSubclass) { //-V809
			delete m_phdrSubclass;
		}
	}
	...
	LRESULT OnNotify(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
		...
		if (hWnd == m_lvw.m_hWnd) {
			LRESULT retval = my::lvHandleNotify(this,
	...
	virtual int getLastSelItemIndex() {
		return m_lastSelItemIndex;
	}
	virtual void setLastSelItemIndex(int idx) {
		m_lastSelItemIndex = idx;
	}
...
#pragma warning(default : 6387)
	...
	HRESULT myCreateControlWindow(
		...
		if (m_lvw.m_hWnd == 0) {
			...
			if (FAILED(hr)) {
				return AtlReportError(CLSID_ListControl,
					L"Listview creation: unable to create internal (selected) listitems",
					IID_IListControl, E_OUTOFMEMORY);
			}
			m_selItems->AddRef();
			...
			this->setHeaderhWnd(ListView_GetHeader(myhWnd));
...
public:
	...
	STDMETHOD(get_SelectedItems)(IListItems** pVal) {
		...
		if (*pVal)
			(*pVal)->Release();
		HRESULT hr = m_selItems->QueryInterface(IID_IListItems, (void**)pVal);
		return hr;
	}
...
private: // for developer use, internally
	...
	void setHeaderhWnd(HWND hWnd) {
		ASSERT(m_phdrSubclass == 0);
		m_hdrWnd = hWnd;
		m_phdrSubclass = new my::win32::Subclasser<CListControl>(this, hWnd);
		this->m_hdr->m_hWnd = hWnd;
		this->m_hdr->applyWidths();
		this->m_hdr->setup(lvhWnd());
	}
	...
	LRESULT OnSubclassProc(
		...
		case HDM_LAYOUT:
			if (m_hdr) {
				...
				if (m_hdr->apiSetHeight(hWnd, msg, wParam, lParam, bHandled)) {
					this->Refresh();
