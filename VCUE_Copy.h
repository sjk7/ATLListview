...
// ATL uses copy policy classes in its enumerator and collection interface
// implementations
//
//////////////////////////////////////////////////////////////////////
...
class GenericCopy {
    ...
    static void init(destination_type* p) { _Copy<destination_type>::init(p); }
...
template <> class GenericCopy<BSTR> {
    public:
    ...
    static void destroy(BSTR* p) {
        SysFreeString(*p); //-V745
        *p = NULL;
    }
    static HRESULT copy(BSTR* pTo, const BSTR* pFrom) {
        *pTo = SysAllocStringByteLen(
            reinterpret_cast<const char*>(*pFrom), SysStringByteLen(*pFrom));
        if (*pTo)
            return S_OK;
        else
...
template <> class GenericCopy<VARIANT, BSTR> {
    public:
...
template <> class GenericCopy<BSTR, VARIANT> {
    public:
    typedef BSTR destination_type;
    ...
    static HRESULT copy(destination_type* pTo, const source_type* pFrom) {
        VARIANT var = {0};
        HRESULT hr
            = VariantChangeType(&var, const_cast<VARIANT*>(pFrom), 0, VT_BSTR);
...
template <> class GenericCopy<VARIANT, IDispatch*> {
    public:
    typedef VARIANT destination_type;
...
class MapCopy {
    ...
    static HRESULT copy(destination_type* pTo, const source_type* pFrom) {
        return GenericCopy<destination_type, pseudosource_type>::copy(
            pTo, &(pFrom->second));
    }
