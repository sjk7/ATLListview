...
CColumnHeaders::CColumnHeaders()
    : m_plv(0), m_col_interface(0), m_scaleUnits(my::win32::pixelUnits), m_hWnd(0), m_heightInPixels(-1), m_hWndLv(0){
...
BOOL CColumnHeaders::apiSetHeight(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
...
int CColumnHeaders::addColumn(const CString& txt, int width, VARIANT* key) {
	...
	ptr->setup(m_plv, m_plv->lvhWnd(), this, ListView_GetHeader(m_plv->lvhWnd()));
    ptr->setListview(m_plv);
	if (width >= 0)
		ptr->m_width = width;

	ptr->applyWidth();
	ptr->m_VBsubItemIndex = (short)idx; // the first column is NOT a subitem, so gets 0 here (rem: VBIndex,right?)
...
STDMETHODIMP CColumnHeaders::Add(VARIANT* Index, VARIANT* Key, VARIANT* Text,
	...
	if (FAILED(hr)) {
		return reportError(L"Bad type in text argument to ColumnHeaders::Add()", DISP_E_TYPEMISMATCH);
	}
	
    int added = addColumn(txt, 100, Key);
    ASSERT(added == index);
    if (m_cols.size() != index + 1) {
		return reportError(L"Unable to add column: probably out of memory", E_OUTOFMEMORY);
    }

	if (Key) {
		if (Key->vt != VT_ERROR) {
			ColumnHeader* ptest = findColumnHeader(Key);
			ASSERT(ptest);
		}
	}
...
STDMETHODIMP CColumnHeaders::Remove(LONG indexToRemove)
{
	...
	IColumnHeader* pcol = 0;
	HRESULT hr = m_cols.at(apiIndex)->QueryInterface(IID_IColumnHeader, (void**)pcol);
