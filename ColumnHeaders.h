...
class ATL_NO_VTABLE CColumnHeaders
	...
	inline HRESULT reportError(const std::wstring& what, HRESULT hr = E_FAIL)  const {
		return my::atlReportError(CLSID_ColumnHeaders, IID_IColumnHeaders, what, hr);
	}

	BOOL apiSetHeight(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam, BOOL& bHandled);
	void applyColHeights() {
		// this does the trick for making the new column height apply right away:
		LVCOLUMN c = { 0 };
		c.mask = LVCF_WIDTH;
		c.cx = 0;
		long was = my::lvGetColumnCount(m_hWndLv);
		long inserted = ListView_InsertColumn(m_hWndLv, m_cols.size() + 1, &c);
