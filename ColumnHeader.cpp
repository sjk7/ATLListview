	...
	if (m_plv) {
		...
		if (::IsWindow(hWnd)) {
			...
			if (apiIndex >= 0 && apiIndex < ncols) {
				...
				ASSERT(ret);
				if (!ret) {
					return reportError(L"Setting columnHeader width: index out of bounds", DISP_E_BADINDEX);
				}
			}
		}
	}
...
STDMETHODIMP ColumnHeader::put_Alignment(ListColumnAlignmentConstants newVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
		...
		if (!m_plv) {
			return reportError(L"Cannot set a value on a ColumnHeader that is not associated with a listview!");
		}
	m_align = newVal;
	LVCOLUMN col = { 0 };
