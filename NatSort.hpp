...
namespace SI
{
	namespace natural
	{
		namespace detail
		{
			...
			__inline bool natural_less(const ElementType &lhs, const ElementType &rhs)
			{
			...
			template<typename ElementType>
			__inline bool is_not_digit(const ElementType &x)
			{
				return !isdigit(x);
			}
			...
			{
				__inline int operator()(const Iterator &lhs, const Iterator &rhs) const
				{
			...
			***************************************************/
			...
			private:
				...
				__inline int fractional(Iterator lhsBegin, Iterator lhsEnd, Iterator rhsBegin, Iterator rhsEnd)
				{
				...
				__inline int non_fractional(Iterator lhsBegin, Iterator lhsEnd, Iterator rhsBegin, Iterator rhsEnd)
				{
					//Skip Inital Zero's
					while (lhsBegin<lhsEnd && *lhsBegin == '0') lhsBegin++;
					while (rhsBegin<rhsEnd && *rhsBegin == '0') rhsBegin++;
		...
		__inline void sort(Container &container)
		{
			std::sort(container.begin(), container.end(), compare<typename Container::value_type>);
		}

		template<typename Iterator>
		__inline void sort(const Iterator &first, const Iterator &end)
		{
			std::sort(first, end, compare<typename Iterator::value_type>);
		}

		template<typename ValueType>
		__inline void sort(ValueType* const first, ValueType* const end)
		{
			std::sort(first, end, compare<ValueType>);
		}

		template<typename ValueType, int N>
		__inline void sort(ValueType container[N])
		{
