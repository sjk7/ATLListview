...
public:
	ColumnHeader()
		...
		, m_fixedWidth(-1) // means we never auto-resize the column. User can resize it still if he wants
		, m_VBsubItemIndex(-1)
		, m_widthInPixels(0)
		, m_lvhWnd(0)
		, m_hWnd(0)
	...
	void setVBIndex(size_t index) { m_VBIndex = index; }
	void setup(CListControl* lv, HWND lvhWnd, CColumnHeaders* hdrs, HWND hWnd) {
		m_plv = lv;
		m_lvhWnd = lvhWnd;
		m_hdrs = hdrs;
		m_hWnd = hWnd;
	...
	inline HRESULT reportError(const std::wstring& what, HRESULT hr = E_FAIL)  const {
		return my::atlReportError(CLSID_ColumnHeader, IID_IColumnHeader, what, hr);
	}
	...
	inline ColumnContentType contentType() const {
...
public:
	...
	STDMETHOD(get_Alignment)(/*[out, retval]*/ ListColumnAlignmentConstants *pVal);
